...
def NoGradParamList(x):
    assert isinstance(x, list)
    return nn.ParameterList(x).requires_grad_(False)
...
def get_imshapes_group(groups, pred_dict):
    # n_imgs = max(max(e) for e in edges) + 1
    n_imgs = max(max(e) for e in groups) + 1
    imshapes = [None] * n_imgs

    for group_id, group in enumerate(groups):
        for i, img_idx in enumerate(group):
            shape = tuple(pred_dict[group_id][i].shape[0:2])
            if imshapes[img_idx]:
                assert imshapes[img_idx] == shape, f'incorrect shape for image {img_idx}'
            imshapes[img_idx] = shape
    return imshapes
