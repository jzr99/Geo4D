...
def get_3D_model_from_scene(outdir, silent, scene, min_conf_thr=3, as_pointcloud=False, mask_sky=False,
                            clean_depth=False, transparent_cams=False, cam_size=0.05, show_cam=True, save_name=None, thr_for_init_conf=True, is_msk=True):
    """
    ...
    # get optimized values from scenee
    rgbimg = scene.imgs # [1, 256, 256, 3]
    ...
    if is_msk:
        ...
        msk = [np.ones_like(rgbimg[i][:, :, :], dtype=np.bool_) for i in range(len(rgbimg))]
