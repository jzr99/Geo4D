...
def depth_evaluation(predicted_depth_original, ground_truth_depth_original, max_depth=80, custom_mask=None, post_clip_min=None, post_clip_max=None, pre_clip_min=None, pre_clip_max=None,
                     ...
                     disp_input=False, align_mask=None, return_st=False):
    """
    ...
    if align_mask is None:
        ...
        if align_with_lstsq:
            ...
            s, t = absolute_value_scaling(predicted_depth[align_mask], ground_truth_depth[align_mask], s=torch.median(ground_truth_depth) / torch.median(predicted_depth))
            predicted_depth = s * predicted_depth + t
        elif align_with_lad2:
            s_init = (torch.median(ground_truth_depth[align_mask]) / torch.median(predicted_depth[align_mask])).item()
            s, t = absolute_value_scaling2(predicted_depth[align_mask], ground_truth_depth[align_mask], s_init=s_init, lr=lr, max_iters=max_iters)
            predicted_depth = s * predicted_depth + t
        elif align_with_scale:
            raise NotImplementedError
            ...
            # Align the predicted depth with the ground truth using median scaling
            scale_factor = torch.median(ground_truth_depth[align_mask]) / torch.median(predicted_depth[align_mask])
            predicted_depth *= scale_factor
