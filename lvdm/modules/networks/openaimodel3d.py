...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fs=None, task=None, **kwargs):
        b,_,t,_,_ = x.shape
        if len(timesteps.shape) == 1:
            original_t = True
        elif len(timesteps.shape) == 2:
            assert timesteps.shape[0] == b
            assert timesteps.shape[1] == t
            timesteps = timesteps.reshape(-1)
            original_t = False
        else:
            raise ValueError('timesteps should be 1D or 2D tensor according to my understanding')
        ...
        if self.task_condition:
            ...
            task_emb = timestep_embedding(task, self.model_channels, max_period=100, repeat_only=False).type(x.dtype)
            task_embed = self.task_embedding(task_emb)
